...
if (!class_exists('MoDatabaseHelper', false))
{
	$dbHelperPath = dirname(__FILE__) . '/DatabaseHelper.php';

	if (is_file($dbHelperPath))
	{
...
class MoLdapUtility
{
	...
	public static function moLdapIsCustomerRegistered()
	{
	...
	public static function moLdapCheckEmptyOrNull( $value )
	{
	...
	public static function moLdapIsCurlInstalled()
	{
	...
	public static function moLdapGetHostname()
	{
		return 'https://login.xecurify.com';
	}

	public static function moLdapGetPluginVersion()
	{
	...
	/**
	 * Get the application input object across Joomla 3–6.
	 *
	 * @param   object|null  $app  Optional application instance.
	 *
	 * @return  object
	 */
	public static function moLdapGetApplicationInput($app = null)
	{
		if ($app === null)
		{
	...
	public static function moLdapEncrypt($str)
	{
		if (!self::moLdapIsExtensionInstalled('openssl'))
		{
			return;
		}
	...
	public static function moLdapDecrypt($value)
	{
		if (!self::moLdapIsExtensionInstalled('openssl'))
		{
			return;
	...
	public static function moLdapIsExtensionInstalled($extensionName)
	{
		if (in_array($extensionName, get_loaded_extensions()))
		{
			return 1;
		}
		else
	...
	public static function moLdapErrorType($errorNumber)
	{
		...
		if ((3 == $errorNumber) || (10301 == $errorNumber))
		{
	...
	public static function updateUserAlreadyExist($userId): bool
	{
		...
		// No update needed if current value is not 0
		return false;
	}

	public static function moLdapGetJoomlaGroups()
	{
	...
	public static function moLdapFetchData($tableName,$condition=true,$method='loadAssoc',$columns='*')
	{
		...
		elseif ($method == 'loadRowList')
		{
			return $db->loadRowList();
		}
		elseif ($method == 'loadAssocList')
		{
	...
	public static function moLdapGetOperatingSystem()
	{
	...
	public static function buildLdapAttributeOptions(array $receivedAttributes = array()): array
	{
		require_once dirname(__FILE__) . '/MoConstants.php';

		$options = MoConstants::LDAP_SERVER_ATTRIBUTES;

		foreach ($receivedAttributes as $attribute)
		{
	...
	public static function formatLdapAttributeValue($attribute, $value)
	{
		$attribute = strtolower((string) $attribute);

		if ($attribute === 'objectguid' || $attribute === 'objectsid')
		{
			if (is_array($value) && isset($value['count']))
			{
				...
				for ($i = 0; $i < $value['count']; $i++)
				{
					if (isset($value[$i]))
					{
						$formatted[] = self::convertBinaryToString($value[$i]);
					}
				}
			...
			return self::convertBinaryToString($value);
		}

		return $value;
	}

	public static function saveTestAuthenticationAttributes($username, $password)
	{
		require_once dirname(__FILE__) . '/MoConstants.php';
		require_once dirname(__FILE__) . '/dirsync_ldap_config.php';
		...
		$adUserDetails = array_map(array(self::class, 'convertBinaryToString'), $authResponse->attributeList ?: array());
		$userDetails = array(
			'Name' => $username,
			'Details' => json_encode($adUserDetails, JSON_UNESCAPED_SLASHES | JSON_PRETTY_PRINT | JSON_INVALID_UTF8_SUBSTITUTE),
		);

		self::moLdapUpdateData(
	...
	public static function exportData($tableNames)
	{
		...
		$jsonData = array();
		$decryptFields = array(
			'#__miniorange_dirsync_config' => array(
				'ldap_server_url',
				'service_account_dn',
		...
		else
		{
			foreach ($tableNames as $tableName)
			{
				...
				try
				{
					...
					if (empty($data))
					{
						$jsonData[$tableName] = array('message' => 'This table is empty.');
					}
					else
					{
						if (isset($decryptFields[$tableName]))
						{
							foreach ($data as $row)
							{
								foreach ($decryptFields[$tableName] as $field)
								{
									if (isset($row->$field) && $row->$field !== '')
									{
										$row->$field = self::moLdapDecrypt($row->$field);
									}
								}
							}
						}
				...
				{
					$jsonData[$tableName] = array('error' => $e->getMessage());
				}
			}
		}
	...
	public static function moLdapIsPluginEnabled($folder, $element)
	{
		$db = MoDatabaseHelper::getDb();
		$query = $db->getQuery(true)
	...
	public static function moLdapGetDetails(string $tablename)
	{
		$db = MoDatabaseHelper::getDb();
		$query = $db->getQuery(true);
		$query->select('*');
