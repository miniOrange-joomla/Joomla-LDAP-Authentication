...
class MoLdapLogger
{
	...
	private static function saveLogToDatabase(string $message, string $type, string $file, int $line, string $function): void
	{
		...
		$columns = array('timestamp', 'log_level', 'message', 'file', 'line_number', 'function_call');
		$values = array($db->quote(date('Y-m-d H:i:s')), $db->quote($type), $db->quote(json_encode($logCode)), $db->quote($file), $db->quote($line), $db->quote($function),);
	...
	private static function getLogCode(string $message): array
	{
		$logDetails = array(
			...
			'Password not provided' => array('code' => 'MOLDAP-A15', 'issue' => 'User did not enter a password.'),
			'Authentication successful.' => array('code' => 'MOLDAP-A09', 'issue' => 'User successfully authenticated.'),
			'LDAP Authentication failed.' => array('code' => 'MOLDAP-A08', 'issue' => 'Invalid credentials or LDAP issue.'),

			// Connection & Configuration Errors
			'LDAP Authentication enable status - false' => array('code' => 'MOLDAP-007', 'issue' => 'LDAP authentication is disabled.'),
			'LDAP bind failed.' => array('code' => 'MOLDAP-A01', 'issue' => 'Failed to bind to the LDAP server.'),
			'LDAP search failed.' => array('code' => 'MOLDAP-020', 'issue' => 'LDAP search operation failed.'),
			'LDAP connection failed.' => array('code' => 'MOLDAP-A02', 'issue' => 'Cannot connect to the LDAP server.'),
			'LDAP extensions disabled.' => array('code' => 'MOLDAP-A22', 'issue' => 'PHP LDAP extension is disabled.'),

			// User Attribute Errors
			'User email not retrieved.' => array('code' => 'MOLDAP-A03', 'issue' => 'Email attribute missing from LDAP response.'),
			'User email attribute that you have contacted is incorrect.' => array('code' => 'MOLDAP-A03', 'issue' => 'Incorrect email attribute configured.'),
			'Username not retrieved. Not getting user\'s username.' => array('code' => 'MOLDAP-A04', 'issue' => 'Username attribute is missing.'),
			'User\'s name not retrieved. Not getting user\'s name.' => array('code' => 'MOLDAP-A05', 'issue' => 'Name attribute is missing.'),
			'Username and Email mismatch.' => array('code' => 'MOLDAP-A10', 'issue' => 'Username and Email mismatch.'),

			// Licensing & Limits
			'Free Login Limit reached.' => array('code' => 'LIC-021', 'issue' => 'Maximum allowed free logins reached.')
		);
		...
		if (preg_match('/MOLDAP[\s-]?A?\d+/i', $message, $matches))
		{
			return array(
				'code' => strtoupper(str_replace(' ', '', $matches[0])),
				'issue' => $message
			);
		}
	...
	{
		$db = MoDatabaseHelper::getDb();
		$query = $db->getQuery(true)->select($db->quoteName(array('timestamp', 'log_level', 'message', 'file', 'line_number', 'function_call')))->from($db->quoteName('#__mo_ldap_logs'))->order($db->quoteName('timestamp') . ' DESC');
